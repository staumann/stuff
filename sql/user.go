...
func (a *userRepo) SaveUser(user *model.User) error {
	res, err := a.adapter.db.Exec(a.adapter.getScript("insert/user"), user.Name)
...
func (a *userRepo) GetUserByID(id int64) *model.User {
	user := new(model.User)
	err := a.adapter.db.QueryRow(a.adapter.getScript("/get/user"), id).Scan(&user.ID, &user.Name)
...
func (a *userRepo) UpdateUser(user *model.User) error {
	...
	if result, e := tx.Exec(a.adapter.getScript("update/user"), user.Name, user.ID); e != nil {
		return e
	} else {
